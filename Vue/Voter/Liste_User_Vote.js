...
export default class Liste_User_Vote extends React.Component {
  ...
  renderSquare(nom, prenom, image) {
    return <Vote_User nom_t_personne={nom} prenom_t_personne={prenom} photo={image} />;
  } 
      ...
      return (
        <View style={{flex: 1}}>
            <ScrollView contentContainerStyle={{flexGrow: 1, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center'}}>
                ...
                return <View style={styles.boxStyle} key={item.id_t_personne}>{this.renderSquare(item.nom_t_personne, item.prenom_t_personne, item.photo)}</View>;
                })}   
            </ScrollView>        
      </View>
      );
